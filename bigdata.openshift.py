...
class OpenShift(object):
    ...
    def NewDcConfig(self, dcconfig):
        ...
        # 判断configmap是否已被挂载
        cmounted = []
        for volume in dcconfig['spec']['template']['spec']['volumes']:
            if 'configMap' in volume.keys():
                if volume['configMap']['name'] == 'common':
                    # print('configMap already mounted common')
                    cmounted.append('common')

                if volume['configMap']['name'] == self.dcname:
                    # print('configMap already mounted %s' %(self.dcname,) )
                    cmounted.append(self.dcname)

        if 'common' not in cmounted:
            if 'common' in cps:
                print('start add configmap common')
                mountinfo = {'readOnly': True, 'mountPath': '/common', 'name': 'common'}
                configinfo = {'configMap': {'defaultMode': 420, 'name': 'common'}, 'name': 'common'}
                ...
                print("common is not exists")

        if self.dcname not in cmounted:
            if self.dcname in cps:
                print('start add configmap %s' % (self.dcname,))
                ...
                print("%s is not exists" % (self.dcname,))
