...
function getServiceBuildUrl(){
	select_sql="select job_url  from devops_deploy.service_list where service_name='$1';"
...
function triggerJenkinsBuild(){
	...
	curl -X POST "$1/buildWithParameters?delay=0sec"  --user $jenkins_username:$jenkins_passwd --silent 
}
...
function getDeployedServices(){
	select_sql="select service_name  from devops_deploy.result where deploy_flag=0;"
...
function getDeployedServicesCount(){
	select_sql="select count(0)  from devops_deploy.result where deploy_flag=0;"
...
function insertResult(){
	...
	insert_sql="INSERT INTO devops_deploy.result(service_name,insert_time,deploy_flag,docker_tag) VALUES('$1','$insert_time',0,$2)"
...
function updateResultFlag(){
	...
	update_sql="UPDATE devops_deploy.result SET deploy_flag = '1' ,update_time = '$update_time' WHERE deploy_flag = '0' "
...
do
	...
	if [[ "true" == "$deployedFlag" ]];then
		...
		if [[ "SUCCESS" == "$currentJobBuildResult" ]]; then
			...
			dockerTag=`getCurrentBuildDockerTag $serviceJenkinsBuildUrl `
...
#当前部署完成后flag全部置为1
updateResultFlag
mv report.txt ./history/`date "+%Y%m%d%H%M"`-report-stg.txt
